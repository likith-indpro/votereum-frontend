...
// Define local types to match those in api.ts
type Election = {
  id: string;
  title: string;
  description: string;
...
export const integrationService = {
  ...
  createElection: async (title: string, description: string) => {
    try {
      console.log("Creating election on blockchain and in database:", {
        title,
        description,
      });

      try {
        // First create the election on the blockchain with updated parameters
        const blockchainResult = await contractUtils.createElection(
          title,
          description
        );
        console.log("Blockchain election result:", blockchainResult);

        const electionAddress = blockchainResult.electionAddress;
        if (!electionAddress) {
          console.error("No election address returned from blockchain transaction");
  ...
  startElection: async (
    ...
    try {
      ...
      const electionDetails =
        await contractUtils.getElectionDetails(electionAddress);
  ...
  castVote: async (
    ...
    try {
      ...
      const hasAlreadyVoted = await contractUtils.hasVoted(
        electionAddress,
        voterAddress
      );
      if (hasAlreadyVoted) {
  ...
  syncElections: async () => {
    try {
      console.log(
        "Synchronizing all elections between blockchain and database"
      );
      ...
      const databaseElectionsByAddress = databaseElections.reduce(
        (map: any, election: Election) => {
      ...
      console.log(
        `Synchronization complete. ${syncedElections.length} elections processed.`
      );
  ...
  syncCandidates: async (electionId: string, electionAddress: string) => {
    try {
      console.log("Synchronizing candidates for election:", {
        electionId,
        electionAddress,
      });
      ...
      for (const blockchainCandidate of blockchainCandidates) {
        ...
        if (candidatesByName[candidateName]) {
          ...
          console.log(
            `Found new candidate on blockchain: ${blockchainCandidate.name}`
          );
      ...
      console.log(
        `Candidate synchronization complete. ${syncedCandidates.length} candidates processed.`
      );
  ...
  getFullElectionDetails: async (electionId: string) => {
    try {
      ...
      if (!electionAddress) {
        throw new Error(
          `Election with ID ${electionId} has no blockchain address`
        );
      }
      ...
      console.error(
        `Error getting full election details for ${electionId}:`,
        error
      );
      throw error;
    }
